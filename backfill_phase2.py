#!/usr/bin/env python3
"""
Backfill Phase 2: Re-embed all documents that have text but missing/stale chunks.
Loads fastembed once and processes all docs — separate from Phase 1 to avoid OOM.
"""
import asyncio, logging, re, sys
import asyncpg
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(message)s",
                    stream=sys.stdout, force=True)
log = logging.getLogger("bf2")
...
async def main():
    ...
    rows = await conn.fetch("""
        ...
        ORDER BY d.id
    """)
    log.info("Phase 2: %d documents need embedding", len(rows))
    for i, row in enumerate(rows, 1):
        log.info("[%d/%d] doc %d", i, len(rows), row["id"])
        try:
