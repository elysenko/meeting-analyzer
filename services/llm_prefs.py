"""
LLM preference helpers — workspace/global preference loading, merging, resolution.

These functions need DB access; callers pass a pool connection.
"""
...
from config import (
    LLM_TASK_KEYS, LLM_RUNNER_CONFIG_PATH, LLM_PROVIDER_LABELS, AGENTIC_LLM_PROVIDERS,
)
...
# ---------------------------------------------------------------------------
# Fallback catalog (reads local YAML config when llm-runner is unavailable)
# ---------------------------------------------------------------------------
...
def _fallback_llm_models() -> dict[str, Any]:
    ...
    if not config_text:
        logger.warning(
            "llm-runner /v1/models unavailable and no local config at %s — returning empty catalog",
...
# ---------------------------------------------------------------------------
# DB-backed global/workspace preference accessors
# ---------------------------------------------------------------------------


async def get_global_llm_defaults(pool) -> dict[str, dict[str, str | None]]:
...
async def set_global_llm_defaults(pool, prefs: dict[str, dict[str, str | None]]) -> dict[str, dict[str, str | None]]:
...
async def get_workspace_llm_preferences(pool, workspace_id: int | None) -> dict[str, dict[str, str | None]]:
    global_defaults = await get_global_llm_defaults(pool)
    if not workspace_id:
